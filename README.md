...
一个赛博朋克风格的 Telegram 文件上传工具。支持视频 (MP4/WebM) 和音频 (MP3/FLAC) 格式，并集成了 Cloudflare KV 历史记录功能和直链代理服务。

## 功能特性
- **赛博朋克 UI**: 沉浸式深色科技风格体验。
- **多格式支持**: 自动识别并发送 Video 或 Audio 消息到 Telegram。
- **历史记录**: 使用 Cloudflare KV 保存并同步最近的上传记录。
- **直链支持**: 提供文件的直连地址 (Stream Proxy)，支持在线播放。
...
### 2. 关键环境变量 (Direct Link 必须)
为了让直链功能 (`/api/file`) 正常工作，你必须添加 Bot Token 到后台环境变量。
1.  Cloudflare Dashboard -> Pages -> Settings -> **Environment variables**.
2.  添加变量:
    - **TG_BOT_TOKEN**: 你的 Telegram Bot Token (例如 `123456:ABC-DEF...`)
3.  **Save**.
...
### 4. 重新部署
修改环境变量或 KV 绑定后，必须手动触发一次 **Retry Deployment** 才能生效。
